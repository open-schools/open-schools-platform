...
class SMSC(object):
    def __init__(self, smsc_login, smsc_password):
        self.SMSC_LOGIN = smsc_login  # логин клиента
        self.SMSC_PASSWORD = smsc_password  # пароль
    ...
    # format - формат сообщения (0 - обычное sms, 1 - flash-sms, 2 - wap-push, 3 - hlr,
    # 4 - bin, 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber, 11 - soc)
    ...
    def send_sms_mail(self, phones, message, translit=0, time="", id=0, format=0, sender=""):
        ...
        server.sendmail(SMTP_FROM, "send@send.smsc.ru", "Content-Type: text/plain; charset=" + SMSC_CHARSET + "\n\n" +
                        self.SMSC_LOGIN + ":" + self.SMSC_PASSWORD + ":" + str(id) +
                        ":" + time + ":" + str(translit) + "," +
    ...
    def _smsc_send_cmd(self, cmd, arg=""):
        ...
        arg = "login=" + quote(self.SMSC_LOGIN) + "&psw=" + quote(
            self.SMSC_PASSWORD) + "&fmt=1&charset=" + SMSC_CHARSET + "&" + arg
...
# smsc.send_sms("79999999999",
# "0605040B8423F0DC0601AE02056A0045C60C036D79736974652E72750001036D7973697465000101", format=5)
